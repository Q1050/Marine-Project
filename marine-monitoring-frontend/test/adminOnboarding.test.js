...
import {
  ...
  getAdminRegions, getOnboardingPreparation, getRegionOnboardingInventory,
  prepareRegionJurisdictions, getAdminRegionTaxa, getAdminJurisdictionTaxonReadiness,
  getAdminTaxa, prepareAdminTaxonomy, approveAdminTaxonomy, applyAdminTaxonomy, getAdminTaxonomyHistory,
} from "../src/services/api.js";
import { canApplyJurisdiction, canPrepareJurisdiction, scientificStateDescription, selectableJurisdictions } from "../src/utils/adminOnboarding.js";
...
test("admin console access requires a platform administrator", () => {
  ...
  assert.equal(hasProtectedAccess({ is_platform_admin: false }, "admin", ["MANAGER"]), false);
  assert.equal(hasProtectedAccess({ is_platform_admin: true }, "admin"), true);
});

function mockResponse(payload = {}, ok = true) {
  globalThis.fetch = async (url, options = {}) => ({ ok, json: async () => payload, url, options });
}
...
test("selection excludes blocked and ambiguous inventory entries", () => {
  ...
  assert.deepEqual(selectableJurisdictions(rows).map((row) => row.canonical_identifier), ["AG", "LC"]);
});

test("scientific empty state remains distinct from geographic onboarding", () => {
  assert.equal(scientificStateDescription("SCIENTIFICALLY_EMPTY"), "No species science inherited");
});
...
test("region and inventory calls use centralized authenticated API client", async () => {
  ...
  await getAdminRegions(); await getRegionOnboardingInventory(1);
  assert.match(calls[0].url, /\/admin\/regions$/);
  assert.match(calls[1].url, /\/admin\/regions\/1\/jurisdiction-onboarding\/prepare\?dry_run=true$/);
...
test("selected preparation is passed as governed canonical identifiers", async () => {
  let call;
  globalThis.fetch = async (url, options = {}) => { call = { url, options }; return { ok: true, json: async () => ({ results: [] }) }; };
...
test("review, approve, single apply, and batch apply target controlled endpoints", async () => {
  ...
  assert.deepEqual(JSON.parse(calls[1].options.body), { expected_manifest_fingerprint: "fingerprint", approval_reference: "review-42" });
  assert.match(calls[2].url, /jurisdiction-onboarding\/7\/apply$/);
  assert.deepEqual(JSON.parse(calls[3].options.body), { preparation_ids: [7, 8] });
});

test("backend validation and conflict reasons are surfaced", async () => {
  mockResponse({ detail: "Approval is stale" }, false);
  await assert.rejects(() => approveOnboardingPreparation(7, "old", "review"), /Approval is stale/);
});

test("taxonomy and jurisdiction readiness use read-only admin endpoints", async () => {
  const calls = [];
  globalThis.fetch = async (url, options = {}) => { calls.push({ url, options }); return { ok: true, json: async () => ({ taxa: [] }) }; };
  await getAdminRegionTaxa(1); await getAdminJurisdictionTaxonReadiness(2, 3);
...
test("controlled taxonomy prepare, approve, apply, and history calls are separated", async () => {
  const calls=[]; globalThis.fetch=async(url,options={})=>{calls.push({url,options});return {ok:true,json:async()=>({})};};
  await getAdminTaxa(); await prepareAdminTaxonomy(1,1); await approveAdminTaxonomy(4,"fp","review"); await applyAdminTaxonomy(4); await getAdminTaxonomyHistory(1);
  assert.match(calls[0].url,/\/admin\/taxa$/); assert.deepEqual(JSON.parse(calls[1].options.body),{taxon_id:1,region_id:1});
  assert.match(calls[2].url,/\/approve$/); assert.match(calls[3].url,/\/apply$/); assert.match(calls[4].url,/\/taxonomy-history$/);
});
