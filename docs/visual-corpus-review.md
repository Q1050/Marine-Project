...
# Taxonomy-review continuation

`TAXONOMY_REVIEW_REQUIRED` assets remain in the Visual Corpus workspace. Administrators can retrieve bounded provider evidence, inspect the evidence history, and record an explicit reasoned resolution. Machine evidence acquisition never impersonates the reviewer and never approves an asset. Review and taxonomy histories are append-only.
